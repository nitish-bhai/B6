...
import { ChatIcon, PencilIcon, TrashIcon, SparklesIcon, ArrowDownTrayIcon, PrinterIcon, ChevronDownIcon, ChevronUpIcon } from './icons';
        ...
        if (sortConfig !== null) {
            sortableItems.sort((a, b) => {
                ...
                if (sortConfig.key === 'expiryDate') {
                    // Sort by timestamp if available for accuracy
                    // Items without expiry are treated as having a very far future date (safe) for asc sort
                    // Use a very large number for null timestamps to push them to the end in asc sort
                ...
                <table className="w-full text-sm text-left text-gray-600 dark:text-gray-300">
                    <thead className="text-xs text-gray-700 dark:text-gray-400 uppercase bg-gray-50 dark:bg-gray-700 dark:bg-opacity-50">
                        <tr>
                            ...
                            {renderSortHeader('Price/Item', 'price', 'right')}
                            {renderSortHeader('Expiry', 'expiryDate')}
                            {renderSortHeader('Total Value', 'totalValue', 'right')}
